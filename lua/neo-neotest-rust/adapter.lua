...
local constructor = function(opts)
    local adapter = {
        ...
        opts = {
            ...
            get_dap_extra_options = function()
                return {}
            end,
        },
    }
    if opts ~= nil then
        if is_callable(opts.args) then
            adapter.opts.get_args = opts.args
        elseif opts.args then
            adapter.opts.get_args = function()
                return opts.args
            end
        end
        if is_callable(opts.dap_adapter) then
            adapter.opts.get_dap_adapter = opts.dap_adapter
        elseif opts.dap_adapter then
            adapter.opts.get_dap_adapter = function()
                return opts.dap_adapter or "codelldb"
            end
        end
        if is_callable(opts.dap_extra_options) then
            adapter.opts.get_dap_extra_options = opts.dap_extra_options
        elseif opts.dap_extra_options then
            adapter.opts.get_dap_extra_options = function()
                return opts.dap_extra_options
            end
        end
    end
    ...
    -- local get_dap_extra_options = function()
    --     return {}
    -- end
    ...
    function adapter.build_spec(args)
        ...
        local command = tbl_flatten({
            ...
            "--profile",
            "neotest",
            vim.list_extend(adapter.opts.get_args(), args.extra_args or {}),
        })
        ...
        if args.strategy == "dap" then
            ...
            local strategy = vim.tbl_deep_extend("force", {
                ...
                type = adapter.opts.get_dap_adapter(),
                request = "launch",
                ...
                adapter.opts.get_dap_extra_options()
            )
            ...
            if adapter.opts.get_dap_adapter() == "codelldb" then
